...
from xrayclassifier.data_utils_nn import KIMDB_Data_Utils
from xrayclassifier.model_nn import KModel
from xrayclassifier.parser_utils_nn import KParseArgs
from xrayclassifier.graphs_nn import KPlot
...
class KTrain():
    ...
    def compile_and_fit_model(self, model, x_train, y_train, epochs=20, batch_size=40, loss='binary_crossentropy',
        ...
        history = model.fit(x_train,
                            y_train,
                            epochs = epochs, batch_size = batch_size, verbose=verbose, validation_split=0.20,callbacks=tensorboard)
    ...
    def train_models(self, args, base_line=True):
        ...
        (train_data, train_labels) = kdata_cls.fetch_imdb_data()
        ...
        history = ktrain_cls.compile_and_fit_model(model, train_data, train_labels, epochs=args.epochs, loss=args.loss, output_dir=output_dir)
        model.summary()
        ...
        with mlflow.start_run():
            ...
            # log model
            mlflow.keras.log_model(model, "models",keras_module='keras')
